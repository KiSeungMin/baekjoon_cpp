...
void goLeft(int level, vector<vector<long long>>board){
    ...
    for(int i{0}; i < N; i++){
        for(int j{1}; j < N; j++){
            ...
            board[i][j] = 0;
            int nextIndex = j - 1;
            while(nextIndex > 0 && board[i][nextIndex] == 0){
                nextIndex--;
            }
...
void goRight(int level, vector<vector<long long>>board){
    ...
    for(int i{0}; i < N; i++){
        for(int j{N - 2}; j >= 0; j--){
            ...
            int nextIndex = j + 1;
            while(nextIndex < N - 1 && board[i][nextIndex] == 0){
                nextIndex++;
            }
            int curIndex = nextIndex - 1;
...
void goUp(int level, vector<vector<long long>>board){
    ...
    for(int j{0}; j < N; j++){
        for(int i{1}; i < N; i++){
            ...
            int nextIndex = i - 1;
            while(nextIndex > 0 && board[nextIndex][j] == 0){
                nextIndex--;
            }
            int curIndex = nextIndex + 1;
...
void goDown(int level, vector<vector<long long>>board){
    ...
    for(int j{0}; j < N; j++){
        for(int i{N - 2}; i >= 0; i--){
            ...
            int nextIndex = i + 1;
            while(nextIndex < N - 1 && board[nextIndex][j] == 0){
                nextIndex++;
            }
            int curIndex = nextIndex - 1;
